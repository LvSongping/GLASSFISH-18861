...
public class RestApiHandlers {
    ...
    public static void createEntity(HandlerContext handlerCtx) {
        ...
        String endpoint = (String) handlerCtx.getInputValue("endpoint");
        String appName = attrs.get("name").toString();
        String contextRoot = attrs.get("contextRoot").toString();
        if(RestUtil.contextroot_confirm(appName,contextRoot)){
            GuiUtil.handleError(handlerCtx, GuiUtil.getMessage("the contextRoot '"+ contextRoot 
                    +"' is Duplicated with application '" + appName 
                    +"' ,please try another contextRoot again"));
        }
